        ...
        <div className="space-y-8 pt-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-2">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/5 border border-white/10 rounded-full text-white text-[11px] font-black uppercase tracking-widest shadow-inner">
                        <Package className="w-3 h-3" />
                        Billing & Plans
                    </div>
                    <h1 className="text-4xl font-black tracking-tight text-white uppercase italic">Manage <span className="grad-text">Credits</span></h1>
                    <p className="text-white font-black text-xs uppercase tracking-widest">Choose a flexible plan that grows with your business.</p>
                </div>
                ...
                <div className="bg-white/5 p-1 rounded-2xl flex gap-1 border border-white/5 shadow-inner">
                    <button
                        ...
                        className={cn(
                            ...
                            billingCycle === 'topup' ? "bg-white text-white shadow-lg" : "text-white hover:text-white"
                        )}
                    >
                        Top-up Packs
                    </button>
                    <button
                        ...
                        className={cn(
                            "px-6 py-2.5 rounded-xl text-[11px] uppercase tracking-widest font-black transition-all",
                            billingCycle === 'subscription' ? "bg-white text-white shadow-lg" : "text-white hover:text-white"
                        )}
                    >
                        Monthly Subs
                    </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {currentPlans.map((plan, i) => (
                    ...
                    >
                        ...
                        {plan.popular && (
                            <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[var(--accent-1)] text-white px-6 py-1.5 rounded-full text-[11px] font-black uppercase tracking-widest shadow-xl shadow-black/20">
                                Most Popular
                            </div>
                        )}
                        ...
                        <div className="space-y-4 mb-8 relative">
                            <div className="flex items-center justify-between">
                                ...
                                {billingCycle === 'topup' ? (
                                    ...
                                    <TrendingUp className="w-6 h-6 text-white animate-glow" />
                                )}
                            </div>
                            <p className="text-white text-[11px] uppercase font-black tracking-widest opacity-60 leading-relaxed">{plan.description}</p>
                            <div className="pt-4 flex items-baseline gap-1">
                                <span className="text-4xl font-black text-white tracking-tighter">
                                    {typeof plan.price === 'number' ? `₹${plan.price}` : plan.price}
                                </span>
                                {billingCycle === 'subscription' && typeof plan.price === 'number' && (
                                    <span className="text-white font-black uppercase tracking-widest text-[11px] opacity-60">/mo</span>
                                )}
                            </div>
                        </div>

                        <div className="flex-1 space-y-4 mb-10 relative">
                            <div className="text-[10px] font-black text-white uppercase tracking-widest border-b border-white/5 pb-2 opacity-50">Included Tech</div>
                            {plan.features.map((feature, idx) => (
                                <div key={idx} className="flex items-start gap-3">
                                    ...
                                    <span className="text-[11px] text-white font-black uppercase tracking-widest opacity-60 leading-tight">{feature}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                plan.popular
                                    ? "bg-white text-white shadow-2xl shadow-indigo-500/10"
                ...
                {billingCycle === 'subscription' && subscriptions.length < 4 && (
                    <div className="bg-white/[0.02] p-8 rounded-[2.5rem] border border-dashed border-white/10 flex flex-col justify-between text-white relative h-full shadow-inner overflow-hidden">
                        <div className="absolute inset-0 bg-grid-white/[0.02] [mask-image:linear-gradient(0deg,white,rgba(255,255,255,0))] -z-10" />
                        <div className="space-y-4">
                            <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center border border-white/5 text-white mb-4">
                                <Building className="w-8 h-8" />
                            </div>
                            <h3 className="text-2xl font-black uppercase tracking-tight italic">White-Label Solution</h3>
                            <p className="text-white text-[11px] uppercase font-black tracking-widest opacity-60 leading-relaxed">
                                Want to sell this as your own product? Get a custom branded dashboard for your agency.
                            </p>
                        </div>
                        <button className="mt-12 w-full py-5 border border-white/10 rounded-2xl font-black uppercase tracking-widest text-[11px] hover:bg-white/5 transition-all text-white active:scale-95">
                            Request HQ Info
                        </button>
                    </div>
                )}
            ...
            <div className="pt-12 grid grid-cols-1 md:grid-cols-3 gap-8 border-t border-white/5">
                {[
                    ...
                    <div key={i} className="flex gap-4 items-start">
                        <div className="bg-white/5 p-3 rounded-[1.2rem] text-white shrink-0 border border-white/5">
                            <item.icon className="w-6 h-6" />
                        </div>
                        <div>
                            <h4 className="font-black text-white uppercase tracking-tight italic text-sm">{item.title}</h4>
                            <p className="text-[11px] text-white font-black uppercase tracking-widest opacity-60 leading-relaxed mt-1">{item.desc}</p>
                        </div>
                    </div>
                ))}
            </div>
